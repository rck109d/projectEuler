...
public class P54_PokerHands {
    ...
    private static String getInput() {
        return "8C TS KC 9H 4S 7D 2S 5D 3S AC\n" +
    ...
    public static void main(String[] args) {
        System.out.println(Hand.countHandOneWins(getInput()));
    }

    private static class Card {
        private static Map<Character, Integer> rankCharToRank = new HashMap<>();

        static {
            rankCharToRank.put('T', 10);
            rankCharToRank.put('J', 11);
            rankCharToRank.put('Q', 12);
            rankCharToRank.put('K', 13);
            rankCharToRank.put('A', 14);
        }
        ...
        Card(String cardString) {
            this.orig = cardString;
            char rankChar = cardString.charAt(0);
            if ('2' <= rankChar && rankChar <= '9') {
                rank = rankChar - '0';
            } else if (rankCharToRank.containsKey(rankChar)) {
                rank = rankCharToRank.get(rankChar);
            } else {
                throw new RuntimeException("invalid rank character: " + rankChar);
            }
    ...
    private static class Hand {
        ...
        private Hand(Collection<Card> c) {
            cards = c;
        }

        static long countHandOneWins(String lines) {
            return Arrays.stream(lines.split("\n")).parallel().map(hands -> {
                List<String> cards = Arrays.asList(hands.split(" "));
                Hand handOne = Hand.fromCardStrings(cards.subList(0, 5));
                Hand handTwo = Hand.fromCardStrings(cards.subList(5, 10));
                return isHandOneWinner(handOne, handTwo);
            }).filter(b -> b).count();
        }

        static boolean isHandOneWinner(Hand one, Hand two) {
            boolean oneHasHighestRank = one.getHighCard().getRank() > two.getHighCard().getRank();
            // true ~ yes; false ~ no; null ~ unclear
            Function<Predicate<Hand>, Boolean> checkWin = (winTest) -> {
                boolean oneWins = winTest.test(one);
                boolean twoWins = winTest.test(two);
                if (oneWins) {
                    return !twoWins || oneHasHighestRank;
                } else if (twoWins) {
                    return false;
                }
                return null;
            ...
            done = checkWin.apply(Hand::isRoyalFlush);
            if (done == Boolean.TRUE) {
                return true;
            } else if (done == Boolean.FALSE) {
                return false;
            }

            done = checkWin.apply(Hand::isStraightFlush);
            if (done == Boolean.TRUE) {
                return true;
            } else if (done == Boolean.FALSE) {
                return false;
            ...
            done = checkWin.apply(Hand::hasFourOfAKind);
            if (done == Boolean.TRUE) {
                return true;
            } else if (done == Boolean.FALSE) {
                return false;
            ...
            done = checkWin.apply(Hand::hasFullHouse);
            if (done == Boolean.TRUE) {
                return true;
            } else if (done == Boolean.FALSE) {
                return false;
            ...
            done = checkWin.apply(Hand::isFlush);
            if (done == Boolean.TRUE) {
                return true;
            } else if (done == Boolean.FALSE) {
                return false;
            ...
            done = checkWin.apply(Hand::isStraight);
            if (done == Boolean.TRUE) {
                return true;
            } else if (done == Boolean.FALSE) {
                return false;
            ...
            final Integer oneTOAKRank = one.getThreeOfAKindRank();
            final Integer twoTOAKRank = two.getThreeOfAKindRank();
            if (oneTOAKRank != null) {
                return twoTOAKRank == null || oneTOAKRank > twoTOAKRank;
            } else if (twoTOAKRank != null) {
                return false;
            }

            final Collection<Integer> onePairRanks = one.getPairRanks();
            final Collection<Integer> twoPairRanks = two.getPairRanks();
            if (onePairRanks.size() == 2) {
                if (twoPairRanks.size() == 2) {
                    int oneMax = onePairRanks.stream().collect(Collectors.summarizingInt(Integer::intValue)).getMax();
                    int twoMax = twoPairRanks.stream().collect(Collectors.summarizingInt(Integer::intValue)).getMax();
                    return oneMax > twoMax;
                }
                return true;
            } else if (twoPairRanks.size() == 2) {
                return false;
            }

            if (onePairRanks.size() == 1) {
                if (twoPairRanks.size() == 1) {
                    int oneMax = onePairRanks.stream().collect(Collectors.summarizingInt(Integer::intValue)).getMax();
                    int twoMax = twoPairRanks.stream().collect(Collectors.summarizingInt(Integer::intValue)).getMax();
                    return oneMax > twoMax;
                }
                return true;
            } else if (twoPairRanks.size() == 1) {
                return false;
            }

            return oneHasHighestRank;
        }

        static Hand fromCardStrings(Collection<String> cardStrings) {
            return new Hand(cardStrings.stream().map(Card::new).collect(Collectors.toList()));
        }

        static Hand fromCards(Collection<Card> c) {
            List<Card> cards = new ArrayList<>();
            cards.sort((left, right) -> left.getRank() - right.getRank());
            return new Hand(c);
        }

        Card getHighCard() {
            if (high == null) {
                high = cards.stream().max(Comparator.comparing(Card::getRank)).get();
            }
            return high;
        }
        ...
        Card getLowCard() {
            if (low == null) {
                low = cards.stream().min(Comparator.comparing(Card::getRank)).get();
            }
            return low;
        }
        ...
        boolean isRoyal() {
            if (royal == null) {
                royal = cards.stream().allMatch(c -> c.rank > 10);
            }
            return royal;
        }
        ...
        boolean isStraight() {
            if (straight == null) {
                final Set<Integer> rankCountKeys = getRankCounts().keySet();
                boolean allUnique = rankCountKeys.size() == 5;
                boolean regularStraight = allUnique && getHighCard().getRank() - getLowCard().getRank() == 4;
                final Integer[] aceLowStraightCardRanks = {14, 2, 3, 4, 5};
                boolean aceLowStraight = Arrays.stream(aceLowStraightCardRanks).allMatch(rankCountKeys::contains);
                straight = regularStraight || aceLowStraight;
            }
            return straight;
        }
        ...
        boolean hasFullHouse() {
            if (fullHouse == null) {
                if (getPairRanks().size() == 1) {
                    int firstPairRank = getRankCounts().entrySet().stream()
                            .filter(entry -> entry.getValue() == 2).findFirst().get().getKey();
                    List<Card> pairCards = cards.stream()
                            .filter(c -> c.getRank() == firstPairRank).collect(Collectors.toList());
                    fullHouse = without(pairCards).getThreeOfAKindRank() != null;
                } else {
                    fullHouse = false;
                }
            }
        ...
        Integer getThreeOfAKindRank() {
            List<Integer> threeOfAKindRanks = getRankCounts().entrySet().stream()
                    .filter(e -> e.getValue() == 3).map(Map.Entry::getKey).collect(Collectors.toList());
        ...
        Collection<Integer> getPairRanks() {
            return getRankCounts().entrySet().stream()
                    .filter(e -> e.getValue() == 2).map(Map.Entry::getKey).collect(Collectors.toList());
        }

        Hand without(Collection<Card> cardsToRemove) {
            Collection<Card> newHandCards = new ArrayList<>(cards);
            newHandCards.removeAll(cardsToRemove);
            return Hand.fromCards(newHandCards);
        }
        ...
        Map<Integer, Long> getRankCounts() {
            if (rankCounts == null) {
                rankCounts = cards.stream()
                        .map(Card::getRank)
